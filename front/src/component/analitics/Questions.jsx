...
function Questions(){
    ...
    if(selectedBriefing){
        fetch("http://localhost:8080/briefing/questions/"+selectedBriefing, {
            mode: 'cors',
            method: 'GET',
        })
        .then(res => res.json())
    ...
    return(
        <div className='studentGroup'>
            <div className="line">
                <h2>Оценка эффективности учебных программАнализ распределения оценок по отдельным вопросам летучек.</h2>
                <div></div>
            </div>
            <div className="form">
                <div className="subject">
                    <p>Предмет</p>
                    <select className='long' name="subject" onChange = {e => setSelectedSubject(e.target.value)} >
                    <option value=""></option>
                ...
                <div className="br">
                    <p>Тема</p>
                    <select className='long' name="br" onChange = {e => setSelectedBriefing(e.target.value)} >
                    <option value=""></option>
                    {x.map((item, index) => 
                        <option value = {item.id} key = {index} >{item.title}</option>
                    )}
